...
class QRStatsCog(commands.Cog):
    ...
    @slash_cmd(
        description="Show QR generation stats (Officers only)",
        contexts={
    ...
    async def qrstats(self, ctx: discord.ApplicationContext) -> None:
        self.logger.debug(
        ...
        if ctx.guild is not None:
            ...
            in_officers = isinstance(member, discord.Member) and self._has_officer_role(member)
        ...
        if not (in_allow or in_officers):
            ...
            await ctx.respond(
                "Only Officers or authorized admins can view stats.",
                ephemeral=True,
            )
            return
